	...
	public class AmplifyImpostor : MonoBehaviour
	{
		...
		private void GenerateTextures( List<TextureOutput> outputList, bool standardRendering )
		{
			...
			if( standardRendering && m_renderPipelineInUse == RenderPipelineInUse.HD )
			{
				...
				m_rtGBuffers[ 3 ] = new RenderTexture( (int)m_data.TexSize.x, (int)m_data.TexSize.y, 16, uformat );
		...
		private static Dictionary<string, AISRPVersions> m_srpVersionConverter = new Dictionary<string, AISRPVersions>()
		{
			...
			{"6.9.2-preview",   AISRPVersions.AI_SRP_6_9_2},
			{"7.0.1",           AISRPVersions.AI_SRP_7_0_1},
			{"7.0.1-preview",   AISRPVersions.AI_SRP_7_0_1},
			{"7.1.1",           AISRPVersions.AI_SRP_7_1_1},
			{"7.1.1-preview",   AISRPVersions.AI_SRP_7_1_1},
			{"7.1.2",           AISRPVersions.AI_SRP_7_1_2},
			{"7.1.2-preview",   AISRPVersions.AI_SRP_7_1_2},
			{"7.1.5",           AISRPVersions.AI_SRP_7_1_5},
			{"7.1.5-preview",   AISRPVersions.AI_SRP_7_1_5},
			{"7.1.6",           AISRPVersions.AI_SRP_7_1_6},
			{"7.1.6-preview",   AISRPVersions.AI_SRP_7_1_6},
		};
		...
		private AISRPVersions GetLowestVersion( string version )
		{
			...
			return AISRPVersions.AI_SRP_7_1_6;
		}
		...
		public void ApplySRP()
		{
			...
			AISRPVersions LWversion = AISRPVersions.AI_SRP_7_1_6;
			AISRPVersions HDversion = AISRPVersions.AI_SRP_7_1_6;
		...
		public void RenderAllDeferredGroups( AmplifyImpostorAsset data = null )
		{
			...
			if( standardRendering )
			{
				...
				{
					// decoding normals
					PackingRemapping( ref m_rtGBuffers[ 1 ], ref m_rtGBuffers[ 1 ], 10 );
			...
			bool specularIsLinear = false;
			if( standardRendering && m_renderPipelineInUse == RenderPipelineInUse.HD && Renderers.Length > 0 && Renderers[ 0 ] != null )
			{
				float surfaceType = Renderers[ 0 ].sharedMaterial.GetInt( "_MaterialID" );
				if( surfaceType == 0 )
				{
					material.SetFloat( "_MaterialType", 1 );
					material.DisableKeyword( "_MATERIALTYPE_STANDARD" );
					material.EnableKeyword( "_MATERIALTYPE_SUBSURFACESCATTERING" );
					specularIsLinear = true;
				} 
				else
				{
					material.SetFloat( "_MaterialType", 0 );
					material.EnableKeyword( "_MATERIALTYPE_STANDARD" );
					material.DisableKeyword( "_MATERIALTYPE_SUBSURFACESCATTERING" );
				}
			}
			...
			{
				if( outputList[ i ].Active )
					ChangeTextureImporter( ref m_rtGBuffers[ i ], m_fileNames[ i ], ( specularIsLinear && i == 1 ) ? false : outputList[i].SRGB, resizeTextures, outputList[ i ].Compression, outputList[ i ].Channels == TextureChannels.RGBA );
			}
			ClearBuffers();
